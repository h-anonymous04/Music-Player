...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def _login(self):
        ...
        if login_challenge:
            challenge_str = try_get(
                login_challenge, lambda x: x[2], compat_str)
...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        itag_qualities = {}
        player_url = None
        q = qualities(['tiny', 'small', 'medium', 'large', 'hd720',
        ...
        if not formats:
            ...
            pemr = try_get(
                ...
                dict) or {}
            reason = get_text(pemr.get('reason')
                              ) or playability_status.get('reason')
        ...
        if pctr:
            ...
            for caption_track in (pctr.get('captionTracks') or []):
                ...
                for translation_language in (pctr.get('translationLanguages') or []):
                    translation_language_code = translation_language.get(
                        'languageCode')
        ...
        if initial_data:
            ...
            if not chapters:
                for engagment_pannel in (initial_data.get('engagementPanels') or []):
                    contents = try_get(
                        engagment_pannel, lambda x: x['engagementPanelSectionListRenderer'][
                    ...
                    for next_num, content in enumerate(contents, start=1):
                        mmlir = content.get(
                            'macroMarkersListItemRenderer') or {}
                ...
                if vpir:
                    ...
                    for tlb in (try_get(
                        ...
                        for getter, regex in [(
                            ...
                            label = (try_get(tbr, getter, dict)
                                     or {}).get('label')
                            if label:
                                mobj = re.match(regex, label)
                                if mobj:
                                    info[mobj.group(
                                        'type') + '_count'] = str_to_int(mobj.group('count'))
                                    break
                    sbr_tooltip = try_get(
...
class YoutubeTabIE(YoutubeBaseInfoExtractor):
    ...
    def _post_thread_entries(self, post_thread_renderer):
        ...
        # inline video links
        runs = try_get(
            post_renderer, lambda x: x['contentText']['runs'], list) or []
        for run in runs:
    ...
    def _rich_grid_entries(self, contents):
        for content in contents:
            video_renderer = try_get(
                content, lambda x: x['richItemRenderer']['content']['videoRenderer'], dict)
            if video_renderer:
                entry = self._video_entry(video_renderer)
                if entry:
                    yield entry
    ...
    def _entries(self, tab, item_id, webpage):
        ...
        if not tab_content:
            return
        slr_renderer = try_get(
            tab_content, lambda x: x['sectionListRenderer'], dict)
        if slr_renderer:
            is_channels_tab = tab.get('title') == 'Channels'
            continuation = None
            slr_contents = try_get(
                slr_renderer, lambda x: x['contents'], list) or []
            for slr_content in slr_contents:
                if not isinstance(slr_content, dict):
                    continue
                is_renderer = try_get(
                    slr_content, lambda x: x['itemSectionRenderer'], dict)
                if not is_renderer:
                    continue
        ...
        visitor_data = try_get(
            context, lambda x: x['client']['visitorData'], compat_str)
        ...
        for page_num in itertools.count(1):
            ...
            if continuation_contents:
                continuation_renderer = continuation_contents.get(
            ...
            on_response_received = dict_get(
                response, ('onResponseReceivedActions', 'onResponseReceivedEndpoints'))
            ...
            if continuation_items:
                ...
                if renderer:
                    ...
                    continue
                renderer = continuation_item.get(
                    'playlistVideoRenderer') or continuation_item.get('itemSectionRenderer')
                if renderer:
    ...
    def _extract_from_tabs(self, item_id, webpage, data, tabs):
        ...
        if renderer:
            ...
            if renderer:
                ...
                if renderer:
                    title = try_get(
                        renderer, lambda x: x['hashtag']['simpleText'])
...
class YoutubeYtBeIE(InfoExtractor):
    _VALID_URL = r'https?://youtu\.be/(?P<id>[0-9A-Za-z_-]{11})/*?.*?\blist=(?P<playlist_id>%(playlist_id)s)' % {
        'playlist_id': YoutubeBaseInfoExtractor._PLAYLIST_ID_RE}
    _TESTS = [{
